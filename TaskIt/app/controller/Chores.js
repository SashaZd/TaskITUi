Ext.define('TaskIt.controller.Chores', {
    ...
    toggleDoneChore : function(t, index, target, record, e, eOpts){
        if (e.direction == 'left' && record.data.is_done == true){
            TaskIt.app.getController('Chores').toggleDoneChoreAfterDirectionCheck(record)
        }
        else if(e.direction == 'right' && record.data.is_done == false){
            TaskIt.app.getController('Chores').toggleDoneChoreAfterDirectionCheck(record)
        }
    },

    toggleDoneChoreAfterDirectionCheck: function(record){
